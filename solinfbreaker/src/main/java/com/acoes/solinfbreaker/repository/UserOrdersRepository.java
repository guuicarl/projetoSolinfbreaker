...
//    List<UserOrders> findByCalculo();//Pegando matches
    ...
    @Query(value = " SELECT * FROM user_orders a, user_orders b " +
            " where a.type <> b.type  " +
            " and a.remaining_value >= b.remaining_value and a.type = 1  and a.id_stock = b.id_stock " +
            " and a.status = b.status and a.status <>2 and b.status <> 2 and a.price <= b.price  and a.remaining_value <>0 and b.remaining_value <> 0  order by a.stock_name , b.id", nativeQuery = true)
    List<UserOrders> findByCalculo();//Pegando matches
...
//    List<UserOrders> testando1(); CASO VOLTAR PRA QUERY USAR ESSE

    @Query(value = " SELECT * FROM user_orders a, user_orders b " +
            " where a.type <> b.type  " +
            " and a.remaining_value <= b.remaining_value and a.type = 1  and a.id_stock = b.id_stock " +
            " and a.status = b.status and a.status <>2 and b.status <> 2 and a.price <= b.price  and a.remaining_value <>0 and b.remaining_value <> 0  order by a.stock_name , a.id", nativeQuery = true)
